...
public: 
    ...
    void draw() const;
    void operator=(const Rectangle &right) //optional to pass by value or by refernce
...
{	
	...
	Rectangle rec=rect;   //copy by initializing                             
	Rectangle rac(rect);  //copy as passing object
	Rectangle recta;      //default use as copy assignment not allowed but     
	recta=rect;           //must use operator= for it. 
    ...
    cout << "Length: " << recta.getLength() << endl;
    cout << "Width: " << recta.getWidth() << endl;
    cout << "Area: " << recta.computeArea() << endl;
    recta.draw();
}
